...
TAKE_PROFIT_PERCENT = 0.005   # 0.5% rise from entry price
SWAP_COST_PER_LOT_PER_NIGHT = -22.0 # Daily swap cost for long positions

# Short Strategy Parameters
SHORT_ENTRY_UPMOVE_INCREMENT = 2.0  # Dollar amount up-move from daily open to trigger short
SHORT_TAKE_PROFIT_TARGET = 1.0    # Dollar amount down-move from short entry for TP
# No SL for shorts as per requirement
# No swap costs for shorts as per requirement
...
def run_backtest(df):
    ...
    if df.empty:
        ...
        return [], STARTING_BALANCE, 0.0, 0.0 # Added default for max_concurrent_mae
    ...
    max_drawdown = 0.0
    max_concurrent_mae = 0.0 # Initialize max_concurrent_mae
    daily_open_price = None
    current_day_str = None
    first_ever_open_price = None
    ...
    if not df.empty:
        ...
        print("Dataframe is empty, cannot set first_ever_open_price.")
        # This case is already handled by the df.empty check below, but good for clarity
    ...
    for index, row in df.iterrows():
        ...
        for position in list(open_long_positions):
            ...
            if candle_high >= position['tp_price']:
                ...
                equity += net_pnl
                if peak_equity > 0:
                    current_drawdown = (peak_equity - equity) / peak_equity if equity < peak_equity else 0.0
                else:
                    current_drawdown = 0.0 if equity >= peak_equity else 1.0
        ...
        for position in list(open_short_positions):
            ...
            if candle_low <= position['tp_price']:
                exit_price = position['tp_price']
                # No swap for shorts
                gross_pnl = (position['entry_price'] - exit_price) * CONTRACT_SIZE * position['size']
                trade_spread_cost = SPREAD_COST_PER_LOT * position['size'] # Currently 0
                net_pnl = gross_pnl + trade_spread_cost
                ...
                trades.append({
                    ...
                    "exit_price": exit_price,
                    "gross_pnl": gross_pnl,
                    "swap_charges": 0.0, # No swap for shorts
                    "spread_cost": trade_spread_cost,
                    "mae": position.get('mae', 0.0),
                    "pnl": net_pnl,
                    "size": position['size'],
                ...
                print(f"  SHORT TP HIT: PosID {position['id']} exited at {exit_price:.5f} (Entry: {position['entry_price']:.5f}, Gross PnL: {gross_pnl:.2f}, Spread: {trade_spread_cost:.2f}, MAE: {position.get('mae', 0.0):.2f}, Net PnL: {net_pnl:.2f})")
        ...
        # --- Calculate Current Concurrent MAE (sum of momentary adverse excursions for all open positions) ---
        momentary_total_adverse_excursion = 0.0
        for pos_long in open_long_positions: # Calculate for open long positions
            # Calculate current adverse excursion for this long position based on candle_low
        ...
        for pos_short in open_short_positions: # Calculate for open short positions
            # Calculate current adverse excursion for this short position based on candle_high
        ...
        if candle_low <= next_entry_target_price:
            ...
            position_id = f"Pos-{len(trades) + len(open_long_positions) + len(open_short_positions) + 1}-{candle_timestamp.strftime('%H%M%S')}"
            ...
            new_long_position = {
                ...
                "size": NUMBER_OF_LOTS,
                "status": "OPEN",
                "daily_open_ref": daily_open_price,
                "mae": 0.0 # Initialize MAE (positive)
            }
            # Initial MAE check for the opening candle (positive)
            ...
            if candle_high >= tp_price: # Check for same-candle TP for long
                ...
                trades.append({
                    ...
                    "entry_price": entry_price,
                    "exit_time": candle_timestamp,
                    "exit_price": exit_price_immediate,
                    "gross_pnl": gross_pnl_immediate,
                    "swap_charges": swap_charges_immediate,
                    "spread_cost": trade_spread_cost_immediate,
                    "mae": mae_immediate,
                    "pnl": net_pnl_immediate,
                    "size": NUMBER_OF_LOTS,
        ...
        # Temporarily removed short entry logic as per user request
        # if first_ever_open_price is not None and candle_high > first_ever_open_price: # Use first_ever_open_price
        #     max_price_movement_from_ref = candle_high - first_ever_open_price # Use first_ever_open_price
        #     num_potential_short_levels = int(max_price_movement_from_ref // SHORT_ENTRY_UPMOVE_INCREMENT)

        #     for i in range(1, num_potential_short_levels + 1):
        #         target_entry_level_for_short = first_ever_open_price + i * SHORT_ENTRY_UPMOVE_INCREMENT # Use first_ever_open_price
                
        #         is_short_already_open_for_this_trigger = False # Renamed for clarity
        #         for short_pos in open_short_positions:
        #             # Check against the static first_ever_open_price and the specific trigger level
        #             if (short_pos.get('basis_reference_price') == first_ever_open_price and
        #                 short_pos.get('trigger_level_price') == target_entry_level_for_short):
        #                 is_short_already_open_for_this_trigger = True
        #                 break
                
        #         if not is_short_already_open_for_this_trigger and candle_high >= target_entry_level_for_short:
        #             # Open new short position
        #             short_entry_price = target_entry_level_for_short 
        #             short_tp_price = short_entry_price - SHORT_TAKE_PROFIT_TARGET
        #             position_id = f"Pos-{len(trades) + len(open_long_positions) + len(open_short_positions) + 1}-{candle_timestamp.strftime('%H%M%S')}"
        ...
        #                 "size": NUMBER_OF_LOTS, 
        #                 "status": "OPEN",
        #                 "basis_reference_price": first_ever_open_price, # Changed key and value
        #                 "trigger_level_price": target_entry_level_for_short,
        #                 "mae": 0.0 
        #             }
        #             # Initial MAE check for the opening candle (positive)
        ...
        #             print(f"  NEW SHORT: PosID {position_id} opened at {short_entry_price:.5f} (TP: {short_tp_price:.5f}, Lots: {NUMBER_OF_LOTS}) based on FirstEverOpen {first_ever_open_price:.5f} at trigger {target_entry_level_for_short:.5f}")

        #             if candle_low <= short_tp_price: # Check for same-candle TP for short
        #                 exit_price_immediate_short = short_tp_price
        #                 trade_spread_cost_immediate_short = SPREAD_COST_PER_LOT * NUMBER_OF_LOTS # Currently 0
        #                 gross_pnl_immediate_short = (short_entry_price - exit_price_immediate_short) * CONTRACT_SIZE * NUMBER_OF_LOTS
        #                 net_pnl_immediate_short = gross_pnl_immediate_short + trade_spread_cost_immediate_short
        ...
        #                     "entry_price": short_entry_price,
        #                     "exit_time": candle_timestamp,
        #                     "exit_price": exit_price_immediate_short,
        #                     "gross_pnl": gross_pnl_immediate_short,
        #                     "swap_charges": 0.0,
        #                     "spread_cost": trade_spread_cost_immediate_short,
        #                     "mae": mae_immediate_short,
        #                     "pnl": net_pnl_immediate_short,
        #                     "size": NUMBER_OF_LOTS,
        #                     "status": "CLOSED_TP_SAME_CANDLE"
        #                 })
        #                 print(f"  SHORT TP HIT (Same Candle): PosID {position_id} exited at {exit_price_immediate_short:.5f} (Gross PnL: {gross_pnl_immediate_short:.2f}, Spread: {trade_spread_cost_immediate_short:.2f}, MAE: {mae_immediate_short:.2f}, Net PnL: {net_pnl_immediate_short:.2f})")
    ...
    if not df.empty:
        ...
        # Process open short positions
        for pos in open_short_positions:
            # No swap for shorts
            gross_pnl = (pos['entry_price'] - last_close_price) * CONTRACT_SIZE * pos['size']
            trade_spread_cost = SPREAD_COST_PER_LOT * pos['size'] # Currently 0
            net_pnl = gross_pnl + trade_spread_cost
            ...
            print(f"  END OF BACKTEST (SHORT): PosID {pos['id']} still open. Entry: {pos['entry_price']:.5f}, Current Price: {last_close_price:.5f}, Unrealized Gross PnL: {gross_pnl:.2f}, Spread: {trade_spread_cost:.2f}, MAE: {pos.get('mae', 0.0):.2f}, Unrealized Net PnL: {net_pnl:.2f}")
    
    return trades, equity, max_drawdown, max_concurrent_mae

def print_results(trades_history, starting_balance, final_equity, max_drawdown_value, max_concurrent_mae_value): # Added max_concurrent_mae_value
    file_output_lines = []
    ...
    if not trades_history:
        no_trades_messages = [
            ...
            f"Max Concurrent MAE: $0.00" # Added for no trades case
        ]
        for msg in no_trades_messages:
            print(msg)
            file_output_lines.append(msg)
        
        # Write to file even if no trades
        output_file_name = "analysis_matrix_output.txt"
        try:
            with open(output_file_name, "w") as f:
                f.write("\n".join(file_output_lines).lstrip()) # lstrip to remove leading newline if first line is \n...
    ...
    # Initialize file_output_lines here as well if trades_history is not empty,
    # but it's already done at the top of the function.
    
    # Ensure file_output_lines is cleared if it was populated by the "no trades" block (though it returns early)
    # This is not strictly necessary due to the return, but for logical completeness if structure changes:
    # if file_output_lines and "No trades" in file_output_lines[0]:
    #     file_output_lines = [] 
    # This is not needed because of the early return in the no_trades_history block.
    ...
    line = "\n--- Backtest Results ---"
    print(line); file_output_lines.append(line)
    line = f"Starting Balance: ${starting_balance:,.2f}"
    print(line); file_output_lines.append(line)
    line = f"Ending Equity: ${final_equity:,.2f}"
    print(line); file_output_lines.append(line)
    line = f"Net P&L (from equity change): ${final_equity - starting_balance:,.2f}"
    print(line); file_output_lines.append(line)
    line = f"Maximum Drawdown: {max_drawdown_value * 100:.2f}%"
    print(line); file_output_lines.append(line)
    line = f"Max Concurrent MAE: ${max_concurrent_mae_value:,.2f}" # Added Max Concurrent MAE display
    print(line); file_output_lines.append(line)
    
    line = f"\n--- Trade Performance ---"
    print(line); file_output_lines.append(line)
    line = f"Total Trades Recorded (incl. open): {total_trades}"
    print(line); file_output_lines.append(line)
    line = f"Total Gross P&L (all trades): ${total_gross_pnl:,.2f}"
    print(line); file_output_lines.append(line)
    line = f"Total Swap Charges (all trades): ${total_swap_charges:,.2f}"
    print(line); file_output_lines.append(line)
    line = f"Total Spread Costs (all trades): ${total_spread_costs:,.2f}"
    print(line); file_output_lines.append(line)
    line = f"Total Net P&L (all trades, from log): ${total_net_pnl:,.2f}"
    print(line); file_output_lines.append(line)
    ...
    if num_closed_trades > 0:
        ...
        avg_gross_pnl_closed = closed_trades_df['gross_pnl'].mean() if 'gross_pnl' in closed_trades_df.columns else 0
        avg_swap_closed = closed_trades_df['swap_charges'].mean() if 'swap_charges' in closed_trades_df.columns else 0
        avg_spread_closed = closed_trades_df['spread_cost'].mean() if 'spread_cost' in closed_trades_df.columns else 0
        avg_mae_closed = closed_trades_df['mae'].mean() if 'mae' in closed_trades_df.columns else 0

        line = f"  Winning Closed Trades (Net P&L > 0): {num_closed_winning}"
        print(line); file_output_lines.append(line)
        line = f"  Win Rate (Closed Trades): {win_rate_closed:.2f}%"
        print(line); file_output_lines.append(line)
        line = f"  Average Gross P&L per Closed Trade: ${avg_gross_pnl_closed:,.2f}"
        print(line); file_output_lines.append(line)
        line = f"  Average Swap Cost per Closed Trade: ${avg_swap_closed:.2f}"
        print(line); file_output_lines.append(line)
        line = f"  Average Spread Cost per Closed Trade: ${avg_spread_closed:.2f}"
        print(line); file_output_lines.append(line)
        line = f"  Average MAE per Closed Trade: ${avg_mae_closed:.2f}" # Ensure MAE is positive
        print(line); file_output_lines.append(line)
        line = f"  Average Net P&L per Closed Trade: ${avg_net_pnl_closed:.2f}"
        print(line); file_output_lines.append(line)
    ...
    if num_still_open > 0:
        ...
        line = f"  Unrealized Net P&L for Open Trades: ${unrealized_net_pnl_open:.2f}"
        print(line); file_output_lines.append(line)
    ...
    # Ensure 'trade_type' is at the beginning if it exists
    if 'trade_type' not in df_trades.columns and 'trade_type' in display_cols:
        display_cols.remove('trade_type') # Should not happen if added correctly
    
    final_display_cols = [col for col in display_cols if col in df_trades.columns or col.replace('_disp','') in df_trades.columns]

    # Define cols_for_print once for both console and file string
    ...
    # DataFrame string for file
    # Using to_string() with similar formatting options for the file.
    # line_width helps control wrapping, though it's not identical to display.width.
    df_string_for_file = df_trades[cols_for_print].to_string(max_rows=None, float_format='{:,.2f}'.format, line_width=1200)
    file_output_lines.append(df_string_for_file) # Append the DataFrame string

    # Existing console print for DataFrame
    ...
    # Save the collected output lines to analysis_matrix_output.txt
    output_file_name = "analysis_matrix_output.txt"
    try:
        with open(output_file_name, "w") as f:
            f.write("\n".join(file_output_lines).lstrip()) # lstrip to remove potential leading newline from first item
        print(f"\nFull analysis matrix also saved to {output_file_name}")
    except Exception as e:
        print(f"\nError saving analysis matrix to {output_file_name}: {e}")

    # Save detailed trade log to CSV (existing logic)
    csv_filename = "backtest_trade_log.csv"
    try:
        # Select columns for CSV, prefer raw datetime objects over formatted strings for entry/exit times
        csv_cols = ['trade_type', 'entry_time', 'entry_price', 'exit_time', 'exit_price', 'size', 
                    'gross_pnl', 'swap_charges', 'spread_cost', 'mae', 'pnl', 'status']
        # Ensure all selected columns exist in df_trades
        cols_to_save = [col for col in csv_cols if col in df_trades.columns]
        
        df_trades_to_save = df_trades[cols_to_save].copy()
        # Ensure MAE is positive for CSV export if it was negative (it should be positive now)
