...
def run_backtest(df):
    ...
    for index, row in df.iterrows():
        ...
        if candle_low <= next_entry_target_price:
            ...
            print(f"  NEW LONG: PosID {position_id_long} opened at {entry_price:.5f} (TP: {tp_price:.5f}, Lots: {NUMBER_OF_LOTS}) based on DailyOpen {daily_open_price:.5f}. Entries today: {entries_today_count + 1}")
            ...
            # Calculate TP for short based on its own entry price and the updated offset
            short_tp_price = short_entry_price - SHORT_TAKE_PROFIT_PRICE_OFFSET
            ...
            new_short_position = {
                ...
                "entry_price": short_entry_price,  # Use the calculated short entry price
                "tp_price": short_tp_price,        # Use the newly calculated TP price
                "size": short_size,
                "status": "OPEN", "daily_open_ref": daily_open_price, "mae": 0.0
            }
            ...
            print(f"  NEW SHORT (Hedge): PosID {position_id_short} opened at {short_entry_price:.5f} (TP: {short_tp_price:.5f}, Lots: {short_size:.2f})")
