...
def run_backtest(df):
    ...
    active_long_trade_group_ids = set() # To store IDs of trades in the current TP group
    current_long_sub_group_opened_count = 0 # New counter for 5-trade sub-groups
    ...
    for index, row in df.iterrows():
        ...
        if candle_low <= next_entry_target_price:
            ...
            # New Sub-Group TP Logic
            if not active_long_trade_group_ids: # If no trades were open, this is a fresh start for everything
                current_long_sub_group_opened_count = 0 # Reset sub-group count

            current_long_sub_group_opened_count += 1

            if current_long_sub_group_opened_count == 1 or (current_long_sub_group_opened_count - 1) % 5 == 0:
                # This is the 1st trade of a new 5-trade sub-group (or the very first trade overall in a sequence)
                current_anchor_long_tp = entry_price + TAKE_PROFIT_PRICE_OFFSET
                print(f"    NEW ANCHOR TP SET by PosID {position_id_long} (Sub-group trade #{current_long_sub_group_opened_count}): {current_anchor_long_tp:.5f}")
            elif current_anchor_long_tp is None: # Safety net if anchor is somehow None mid-sequence
                current_anchor_long_tp = entry_price + TAKE_PROFIT_PRICE_OFFSET
                print(f"    SAFETY NET ANCHOR TP SET for PosID {position_id_long}: {current_anchor_long_tp:.5f}")

            tp_price_for_this_trade = current_anchor_long_tp
            ...
            print(f"  NEW LONG: PosID {position_id_long} opened at {entry_price:.5f} (TP: {tp_price_for_this_trade:.5f}, Anchor TP: {current_anchor_long_tp:.5f}, SubGrp# {current_long_sub_group_opened_count}, Lots: {NUMBER_OF_LOTS}) based on DailyOpen {daily_open_price:.5f}. Entries today: {entries_today_count + 1}")
            ...
            # Simplified Anchor TP Logic for Shorts
            if current_anchor_short_tp is None: # If no anchor exists (i.e., first trade of a new cluster)
                current_anchor_short_tp = short_entry_price - SHORT_TAKE_PROFIT_PRICE_OFFSET
                print(f"    NEW SHORT CLUSTER ANCHOR TP SET by PosID {position_id_short}: {current_anchor_short_tp:.5f}")
            # All trades in the current cluster use this current_anchor_short_tp
            
            tp_price_for_this_short_trade = current_anchor_short_tp
            ...
            active_short_trade_group_ids.add(position_id_short)
            print(f"  NEW SHORT (Hedge): PosID {position_id_short} opened at {short_entry_price:.5f} (TP: {tp_price_for_this_short_trade:.5f}, Anchor TP: {current_anchor_short_tp:.5f}, Lots: {short_size:.2f})")
